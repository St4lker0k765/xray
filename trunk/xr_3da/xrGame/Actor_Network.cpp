...
void CActor::net_Export	(NET_Packet& P)					// export to server
{
	...
	P.w_angle8			(angle_normalize(r_model_yaw)); //Device.vCameraDirection.getH());//
	P.w_angle8			(angle_normalize(unaffected_r_torso.yaw));//(r_torso.yaw);
	P.w_angle8			(angle_normalize(unaffected_r_torso.pitch));//(r_torso.pitch);
	P.w_angle8			(angle_normalize(unaffected_r_torso.roll));//(r_torso.roll);
	P.w_u8				(u8(g_Team()));
	P.w_u8				(u8(g_Squad()));
	P.w_u8				(u8(g_Group()));
