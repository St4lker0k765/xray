...
void CUITradeWnd::FillList	(TIItemContainer& cont, CUIDragDropListEx& dragDropList, bool do_colorize)
{
	...
	for(; it != it_e; ++it) 
	{
		...
		if(do_colorize)				ColorizeItem		(itm, CanMoveToOther(*it), it);
...
void CUITradeWnd::ColorizeItem(CUICellItem* itm, bool b, TIItemContainer::iterator it)
{
	if(!b)
	...
	else
	{
		...
		{
		case eItemPlaceSlot:
			itm->SetColor(0xff7c0000);
			break;
		case eItemPlaceBelt:
			itm->SetColor(0xff007c00);
			break;
		}
	}
}
